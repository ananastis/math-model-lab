...
def y_linear(x1, x2, t, c):
    """Лінійна функція. Усі другі похідні нулі, отже L(y)=0"""
    return x1 + x2 + c * t

def y_sin_cos(x1, x2, t, c):
    """sin(x1)·cos(x2)·cos(c·t·√2) — точно задовольняє L(y)=0"""
...
TEST_FUNCTIONS = {
    ...
    "sin_cos":       (y_sin_cos,          "sin(x1)·cos(x2)·cos(c·t·√2)"),
...
_EPS_CONE   = 1e-1   # мінімальний disc, нижче якого G = 0
_SMOOTH_W   = 10.0   # ширина зони плавного загасання (×_EPS_CONE)

def green(x1, x2, t, x1s, x2s, ts, c, eps=_EPS_CONE):
    ...
    smooth_thresh = eps * _SMOOTH_W   # верхня межа зони загасання

    if x1.ndim == 0:                  # scalar
        dt_f, disc_f = float(dt), float(disc)
        if dt_f <= 0 or disc_f <= eps:
            return 0.0
        raw = 1.0 / (2.0 * np.pi * c * np.sqrt(disc_f))
        if disc_f < smooth_thresh:
            alpha = (disc_f - eps) / (smooth_thresh - eps)
            return raw * alpha
        return raw
    else:                             # array
        ...
        if not np.any(valid):
            return out
        d_v   = disc[valid]
        raw   = 1.0 / (2.0 * np.pi * c * np.sqrt(d_v))
        # плавне загасання поблизу конуса
        alpha = np.where(
            d_v < smooth_thresh,
            (d_v - eps) / (smooth_thresh - eps),
            1.0
        )
        out[valid] = raw * alpha
        return out
...
def make_sources(x1a, x1b, x2a, x2b, M, c=1.0, T=1.0):
    # Шари розміщені так, щоб конус кожного джерела надійно охоплював
    # область [x1a,x1b]×[x2a,x2b]×[0,T], і жоден не потрапляв на конус
    # спостережень (t=0, межі).
    t_layers = np.linspace(-3.0, -1.5, 5)   # було: linspace(-2.0, -0.2, 4)

    srcs = []
    M_per_layer = max(1, M // len(t_layers))
    n = max(2, int(np.ceil(np.sqrt(M_per_layer))))

    margin = 0.3                             # було: 0.5 (зайво великий)
    ...
    # Якщо M більший за кількість вузлів — дозаповнюємо випадково,
    # але в тих самих безпечних шарах (без шару t=-0.2).
    rng = np.random.default_rng(42)
    while len(srcs) < M:
        ts = float(rng.choice(t_layers))
        x1 = float(rng.uniform(x1a - margin, x1b + margin))
        x2 = float(rng.uniform(x2a - margin, x2b + margin))
...
def solve(A, Y, lam=0.01):
    ...
    # Адаптивний λ: якщо система погано обумовлена — збільшуємо регуляризацію
    lam_eff = lam
    if cond > 1e8:
        lam_eff = max(lam, 1e-1)
        print(f"  → cond > 1e8: λ підвищено до {lam_eff:.1e}")
    elif cond > 1e6:
        lam_eff = max(lam, 1e-3)
        print(f"  → cond > 1e6: λ підвищено до {lam_eff:.1e}")
    elif cond > 1e4:
        lam_eff = max(lam, 1e-4)
        print(f"  → cond > 1e4: λ підвищено до {lam_eff:.1e}")
...
def reconstruct(x1_grid, x2_grid, t_val, srcs, u, c, scale=None):
    X1, X2 = np.meshgrid(x1_grid, x2_grid, indexing='ij')
    Yp      = np.zeros_like(X1)
    clip_val = (10.0 * scale) if (scale is not None and scale > 0) else 1e3

    for m, s in enumerate(srcs):
        contrib = green(X1, X2, t_val, s["x1"], s["x2"], s["t"], c) * u[m]
        # Відкидаємо аномальні внески (артефакти сингулярності)
        contrib = np.clip(contrib, -clip_val, clip_val)
        Yp += contrib
